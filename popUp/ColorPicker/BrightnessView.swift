...
class BrightnessView: UIView {
    ...
    init(frame: CGRect, color: UIColor) {
        ...
        //MARK: - height of brightness bar
        colorLayer.frame = CGRect(x: 0, y: 2, width: self.frame.size.width, height: 13)
    ...
    func touchHandler(_ touches: Set<UITouch>) {
        ...
        //MARK:--centro del circulito
        point.y = self.frame.height/3
    ...
    func drawIndicator() {
        ...
        // Draw the indicator
        if (point != nil) {
            indicator.path = UIBezierPath(arcCenter: point, radius: 17, startAngle: 0, endAngle: 360, clockwise: true).cgPath
            
            indicator.borderWidth = 5
            indicator.borderColor = UIColor.white.cgColor
    ...
    func getPointFromColor(_ color: UIColor) -> CGPoint {
        ...
        //MARK:- centro del circulito
        return CGPoint(x: brightness * frame.width, y: frame.height / 3)
    }
